%matrices to store the accuracy metric
...
for i=1:size(classes,2)
    ...
    % Calculate precision
    PrecisionVals(i,1)=tp/(tp+fp);
    % Calculate Recall
    RecallVals(i,1)=tp/(tp+fn);
    % Calculate f1-score
    F1scores(i,1)=2*RecallVals(i,1)*PrecisionVals(i,1)/(PrecisionVals(i,1)+RecallVals(i,1));
end
